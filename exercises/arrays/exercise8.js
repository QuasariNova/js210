...
// prototype.slice and Array.prototype.splice methods according to the
...
function slice(array, begin, end) {
  ...
  for(let i = begin; i < end; ++i) out.push(array[i]);
...
function splice(array, start, deleteCount, ...args) {
  ...
  deleteCount = start + deleteCount > length ? length - start : deleteCount;
  const end = slice(array, start + deleteCount, length);
  const out = slice(array, start, start + deleteCount);
