...
The issue lies in the fact that arrays are reference types, thus can not be compared with operators. The `===` will only tell you if they are the same object, not if their values are equal.
